...
function edgePoint(name,gx,gy,V){
  // V(x,y) = continuous coverage (0..1) at a grid corner (raw lit/total
  // ratio, BEFORE the 50% hard threshold). Places the crossing at the true
  // sub-pixel position where coverage actually crosses 0.5, instead of
  // always snapping to the cell's exact midpoint — so soft/antialiased
  // edges get traced precisely from the source data, rather than relying
  // on Curve Smoothing afterward to paper over a coarse "always halfway"
  // grid. Falls back to the exact midpoint (0.5) when V is omitted, and
  // clamps away from the corners so a near-flat pair of values can't
  // collapse two edge points onto (near-)the same spot.
  const lerp=(va,vb)=>{
    if(!V) return 0.5;
    const d=vb-va;
    if(Math.abs(d)<1e-4) return 0.5;
    const t=(0.5-va)/d;
    return Math.min(0.9, Math.max(0.1, t));
  };
  if(name==='T') return [gx+lerp(V?V(gx,gy):0, V?V(gx+1,gy):0), gy];
  if(name==='R') return [gx+1, gy+lerp(V?V(gx+1,gy):0, V?V(gx+1,gy+1):0)];
  if(name==='B') return [gx+lerp(V?V(gx,gy+1):0, V?V(gx+1,gy+1):0), gy+1];
  return [gx, gy+lerp(V?V(gx,gy):0, V?V(gx,gy+1):0)];
}
function marchingSquares(mask,w,h,coverage){
  // coverage: optional continuous (0..1) field, same w×h as mask, used only
  // to refine WHERE on each cell edge the crossing lands (see edgePoint).
  // Falls back to the binary mask itself when omitted — interpolation
  // between a 0/1 pair always lands exactly at 0.5, identical to the old
  // fixed-midpoint behavior, so every existing caller is unaffected.
  const cov = coverage || mask;
  const S=(x,y)=> (x<0||y<0||x>=w||y>=h) ? 0 : mask[y*w+x];
  const V=(x,y)=> (x<0||y<0||x>=w||y>=h) ? 0 : cov[y*w+x];
  const segs=[];
  for(let gy=-1; gy<h; gy++){
    for(let gx=-1; gx<w; gx++){
      ...
      for(const [a,b] of pairs) segs.push([edgePoint(a,gx,gy,V), edgePoint(b,gx,gy,V)]);
    }
  }
...
function panelCutoutPaths(panelMask, w, h, dimW, dimD, coverage){
  const polys = marchingSquares(panelMask, w, h, coverage);
...
function getCutoutPolygonsMM(panelData, dimW, boxD){
  const cutPolys = marchingSquares(panelData.mask, panelData.w, panelData.h, panelData.coverage);
